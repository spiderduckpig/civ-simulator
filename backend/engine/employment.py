...
def _wage_cost_per_level_from_professions(city, professions: dict[str, int]) -> float:
    """Calculate total wage cost per staffed level for a profession breakdown."""
    wages = getattr(city, "profession_wages", None) or {}
    total_cost = 0.0
    for prof, count_per_level in professions.items():
        wage = wages.get(prof, 0.0)
        total_cost += count_per_level * wage
    return total_cost


def _building_profit_per_level(city, bkey: str) -> float:
    """Gold/tick revenue minus input costs and wages for one staffed building level.
    
    Profit now accounts for the wage cost of staffing the building, which properly
    couples investment decisions to labor market dynamics.
    """
    if bkey == "trading_house":
        staffed = max(1, int((getattr(city, "building_staffing", None) or {}).get("trading_house", 0) or 0))
        total_profit = float((getattr(city, "building_profit", None) or {}).get(
            "trading_house",
            getattr(city, "trade_export_income", 0.0) or 0.0,
        ))
        per_level = total_profit / staffed

        provided = float(getattr(city, "trade_capacity_provided", 0.0) or 0.0)
        required = float(getattr(city, "trade_capacity_required", 0.0) or 0.0)
        if provided > 0.0 and required > provided * 0.9:
            # If the city is saturating its trade capacity, additional trade
            # house levels are worth more than the current average level.
            per_level *= min(2.0, required / provided)
    ...
    wage_v = _wage_cost_per_level_from_professions(city, b.professions)
    return out_v - in_v - wage_v


def _producer_profit_per_level(city, key: str) -> float:
    """Profit for one staffed level of a producer building, net of wages."""
    ...
    wage_v = _wage_cost_per_level_from_professions(city, meta.get("professions", {}))
    return out_val - in_val - wage_v
...
def update_city_consumption_state(city) -> None:
    """Update per-profession wages from actual building profitability.

    Wages are now derived from the actual profit generated by buildings
    and improvements that employ each profession. This couples wage costs
    to production economics: high labor demand → tight labor → high wages
    → higher production costs → lower margins → investment slows naturally.
    """
    ...
    if not prof_counts and unemployed_count <= 0:
    ...
    # Calculate per-profession profitability from all staffed buildings.
    prof_total_profit: dict[str, float] = {}
    blevels = getattr(city, "buildings", None) or {}
    bstaff = getattr(city, "building_staffing", None) or {}
    ...
    for bkey, lvl in blevels.items():
        ...
        # Get staffed levels for this building (actual staffed, not total capacity).
        staffed = int(bstaff.get(bkey, 0))
        if staffed <= 0:
            continue
        
        # Calculate profit per staffed level.
        if bkey in PRODUCER_BUILDINGS:
            profit_per_level = _producer_profit_per_level(city, bkey)
            profs = PRODUCER_BUILDINGS[bkey].get("professions", {})
        else:
            b = BUILDING_TYPES.get(bkey)
            if b is None or not b.staffable:
                continue
            profit_per_level = _building_profit_per_level(city, bkey)
            profs = b.professions
        
        # Distribute this building's profit to its professions.
        total_per_level = sum(profs.values())
        if total_per_level <= 0:
            continue
        
        for prof, per_level_count in profs.items():
            # This profession makes up (per_level_count / total_per_level) of the building.
            prof_share = per_level_count / total_per_level
            prof_profit_contribution = profit_per_level * prof_share * staffed
            prof_total_profit[prof] = prof_total_profit.get(prof, 0.0) + prof_profit_contribution
    
    # Wage pool is the sum of all profession profits.
    wage_pool = sum(prof_total_profit.values())
    
    # Slack stimulus: add to wage pool if unemployment is high and gold reserves exist.
    ...
    # Fallback: if profitability is negative but income is positive, use
    # income as a safety net to prevent collapse.
    if wage_pool <= 0.0 and total_income > 0.0:
        wage_pool = total_income * PROFESSION_WAGE_POOL_SHARE
    
    # Distribute wages proportionally by profession count and income weight.
    ...
    for prof, count in prof_counts.items():
        ...
        # Hysteresis on affordability: consumption levels move slowly.
    ...
    # Unemployed: if the government actually paid benefits this tick,
    # expose that as their effective wage; otherwise they receive zero.
    if unemployed_count > 0:
        paid = float(getattr(city, "last_unemployment_benefit_per_person", 0.0) or 0.0)
        if paid > 0.0:
            city.profession_wages["unemployed"] = paid
            # Mark income share for unemployed as zero here since the
            # benefit is an explicit government transfer, not part of the
            # profit-derived wage pool used above.
            city.profession_income_shares["unemployed"] = 0.0
        else:
            city.profession_wages["unemployed"] = 0.0
            city.profession_income_shares["unemployed"] = 0.0
        city.consumption_levels.setdefault(
            "unemployed",
            PROFESSION_CONSUMPTION_PROFILES.get("unemployed", {}).get("base_level", 0)
        )
        # Clear the temporary per-person benefit so it doesn't persist
        # beyond the tick it was applied in.
        if hasattr(city, "last_unemployment_benefit_per_person"):
            delattr(city, "last_unemployment_benefit_per_person")
